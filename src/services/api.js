...
export const getTasks = async () => {
  try {
    ...
    // Procesamos la respuesta
    const data = await response.json();
    console.log(`Se obtuvieron ${data.length} tareas en total`);
    
    // Ensure all tasks have userId as string for consistency
    ...
    if (processedData.length > 0) {
      console.log('Primeras 3 tareas (con userId procesado):',
        processedData.slice(0, 3).map(t => ({
          id: t._id,
          title: t.title,
          userId: t.userId,
...
export const assignTask = async (taskData) => {
  try {
    ...
    const userIdString = String(taskData.userId);
    console.log(`Tipo de userId: ${typeof taskData.userId}, convertido a string: ${userIdString}`);
    ...
    const options = {
      ...
      body: JSON.stringify({
        ...taskData,
        userId: userIdString // Ensure userId is sent as a string
      })
    };
    
    console.log('Enviando solicitud para asignar tarea con datos:', options.body);
    ...
    console.log('Tarea asignada correctamente, respuesta:', JSON.stringify(responseData));
    
    // Ensure the returned data has the userId as a string
    if (responseData && responseData.userId) {
      responseData.userId = String(responseData.userId);
    }
    ...
    return {
      task: {
        ...
        userId: responseData.userId || userIdString // Ensure userId is preserved
      }
    };
  } catch (error) {
